...
You have an API endpoint that failed fuzzing/spec testing (e.g., 400 vs 405 HTTP status mismatch, OpenAPI schema error, 500 server error).
1. Extract unique path segments or resource names from the `test_case` URL (e.g., for '/api/v1/tenants/{tenantId}/products', search for 'products' or 'tenantId').
2. DO NOT search the full endpoint URL as a single string if it fails. Search for distinct annotation paths (e.g., @PostMapping, @GetMapping) or Controller class names.
3. If the error is a 405 vs 400 status code mismatch, use `search_files` to find Global Exception Handlers (@ControllerAdvice, ExceptionHandler) or Web Security/Mvc Configs in addition to the Controller.
4. If the error is a Schema Error (e.g., missing path parameter in spec), inspect Swagger/OpenAPI annotations (e.g., @Operation, @Parameter, @PathVariable) in the Controller."""
...
1. Status Code / Spec Mismatches: If Schemathesis expected 405 Method Not Allowed but the server returned 400 Bad Request (or missing OpenAPI parameters), this is an API Specification Compliance Defect (TRUE POSITIVE).
2. Proper Client Validation: Standard 4xx responses (400, 422) returned for invalid request bodies/payloads that MATCH the OpenAPI spec are expected behaviors and classify as False Positives.
3. Safe Server Error Catching: 500 errors are False Positives ONLY IF they are cleanly handled without crashing the service or leaking stack traces.
4. Business Logic / Auth Bypass: If the fuzzed payload bypasses authorization or manipulates state, it is a True Positive."""
...
- [Syntax Check]: Did Phase 1 locate the relevant API Controller, OpenAPI/Swagger annotations, OR Global Exception Handler / Router configuration? If the audit trail lacks inspection of ANY of these for the target endpoint, it is INVALID.
- [Semantic Check]: Verify that the inspected logic or annotation directly explains the root cause of the Schemathesis failure (e.g., missing @PathVariable in Swagger spec, or missing 405 exception mapping in global handler)."""
...
def node_phase7_reporter(state: SecurityState) -> dict:
    ...
    if scan_tool in ["SAST", "CodeQL"]:
        ...
        if scan_tool == "Schemathesis" and "test_case" in vuln_data:
            target = vuln_data.get("test_case")
            vuln_name = vuln_data.get("failure_type", "API Spec / Fuzzing Failure")
        else:
